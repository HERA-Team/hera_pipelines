...
# 10 - path_to_a_priori_flags: folder containing a priori flag YAML files
...
# run redcal
echo redcal_run.py ${uvh5_fn} --ant_z_thresh ${ant_z_thresh} --solar_horizon ${solar_horizon} --oc_maxiter ${oc_maxiter} \
    --flag_nchan_low ${flag_nchan_low} --flag_nchan_high ${flag_nchan_high} --nInt_to_load ${nInt_to_load} \
    --min_bl_cut ${min_bl_cut} --max_bl_cut ${max_bl_cut} --a_priori_ex_ants_yaml ${ex_ants_yaml} \
    --max_dims ${max_dims} --check_every ${check_every} --check_after ${check_after} --clobber --verbose
redcal_run.py ${uvh5_fn} --ant_z_thresh ${ant_z_thresh} --solar_horizon ${solar_horizon} --oc_maxiter ${oc_maxiter} \
    --flag_nchan_low ${flag_nchan_low} --flag_nchan_high ${flag_nchan_high} --nInt_to_load ${nInt_to_load} \
    --min_bl_cut ${min_bl_cut} --max_bl_cut ${max_bl_cut} --a_priori_ex_ants_yaml ${ex_ants_yaml} \
    --max_dims ${max_dims} --check_every ${check_every} --check_after ${check_after} --clobber --verbose
