...
# remove final calibrated dpss inpainted
rm -rfv zen.???????.?????.sum.final_calibrated.dpss_filled.uvh5
# remove final calibrated dpss residual (we still have the xtalk filtered version)
rm -rfv zen.???????.?????.sum.final_calibrated.dpss_res.uvh5
