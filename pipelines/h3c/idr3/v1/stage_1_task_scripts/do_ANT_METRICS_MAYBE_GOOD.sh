...
# 5 - path_to_observer_bad_ants: folder containing text files with filename `{JD}.txt` with bad antennas
# 6+ - filenames
crossCut=${1}
deadCut=${2}
Nbls_per_load=${3}
extension=${4}
path_to_observer_bad_ants=${5}
fn1=`basename ${6}`
data_files="${@:6}"
...
# make filename
bad_ants_fn=`echo "${path_to_observer_bad_ants}/${jd_int}.txt"`
