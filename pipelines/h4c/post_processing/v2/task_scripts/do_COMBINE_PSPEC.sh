...
for sd in ${sumdiff[@]}
do
  fragments=`echo zen.${int_jd}.*.${label}.xtalk_filtered_waterfall.pspec.h5`
  input=zen.${jd}.${sd}.${label}.xtalk_filtered_waterfall.pspec.h5
  combined=zen.${int_jd}.${sd}.${label}.xtalk_filtered_waterfall.pspec.h5
  i=0
  for f in zen.${int_jd}.*.${label}.xtalk_filtered_waterfall.pspec.h5; do
     if [[ "${f}" = "${input}" ]]; then
         fragment_position="${i}";
     fi
     i=$(($i+1))
