...
do
  ...
  do
    ...
    output_file=zen.${jd}.${sd}.${label}.${ext}.filled_flags.xtalk_filtered.chunked.uvh5
    if [ -e "${input_file}" ]
    then
        # transfer flags from res file to model file.
        echo transfer_flags.py zen.${jd}.${sd}.${label}.foreground_filled.xtalk_filtered.chunked.uvh5 ${input_file} ${output_file} --clobber
        transfer_flags.py zen.${jd}.${sd}.${label}.foreground_filled.xtalk_filtered.chunked.uvh5 ${input_file} ${output_file} --clobber
    else
