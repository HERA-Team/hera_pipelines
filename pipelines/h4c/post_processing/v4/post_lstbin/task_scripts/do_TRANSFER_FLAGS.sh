...
then
  exts=("foreground_filled" "foreground_model")
  template_ext="foreground_res"
  flag_label="res_flags"
fi
...
do
  ...
  do
    ...
    output_file=zen.${jd}.${sd}.${label}.${ext}.${flag_label}.xtalk_filtered.chunked.uvh5
    if [ -e "${input_file}" ]
    then
        # transfer flags from res file to model file.
        echo transfer_flags.py zen.${jd}.${sd}.${label}.${template_ext}.xtalk_filtered.chunked.uvh5 ${input_file} ${output_file} --clobber
        transfer_flags.py zen.${jd}.${sd}.${label}.${template_ext}.xtalk_filtered.chunked.uvh5 ${input_file} ${output_file} --clobber
    else
