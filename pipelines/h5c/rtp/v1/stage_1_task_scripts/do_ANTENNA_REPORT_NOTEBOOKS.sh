...
antennas=`python -c "from pyuvdata import UVData; import numpy as np; uv = UVData(); uv.read('${fn}', read_data=False); print('
'.join([str(ant).zfill(3) for ant in (set(uv.ant_1_array) | set(uv.ant_2_array))]))"`
