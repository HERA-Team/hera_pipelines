...
# This version runs on reinpainted data and outputs to a separate folder.
...
jd=$(get_int_jd ${fn})
nb_outdir=${nb_output_repo}/full_day_systematics_inspect_post_reinpaint
nb_outfile=${nb_outdir}/full_day_systematics_inspect_post_reinpaint_${jd}.html

# Export variables used by the notebook
export SUM_FILE="$(cd "$(dirname "$fn")" && pwd)/$(basename "$fn")"
export RED_AVG_SUFFIX="sum.smooth_calibrated.red_avg.uvh5"
export POST_CORNER_TURN_EXTENSION="reinpainted.uvh5"
...
echo Finished full-day systematics inspect post-reinpaint notebook at $(date)
