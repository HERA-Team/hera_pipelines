...
# This script generates an HTML version of a notebook which performs fifth-round full-day RFI flagging
# based on FRF-filtered pI SNRs, both coherently rephased to known bright sources and incoherently combined.
...
jd=$(get_int_jd ${fn})
nb_outdir=${nb_output_repo}/full_day_rfi_round_5
nb_outfile=${nb_outdir}/full_day_rfi_round_5_${jd}.html
...
export SNR_SUFFIX=".inpainted.pI_FRF_SNR.uvh5"
export OUTFILE_SUFFIX=".flag_waterfall_round_5.h5"
...
--execute ${nb_template_dir}/full_day_rfi_round_5.ipynb
echo Finished full-day rfi round 5 notebook at $(date)

# Check to see that at least one per-source flag waterfall was produced
outfile_dir="$(cd "$(dirname "$fn")" && pwd)/single_baseline_files"
n_outfiles=$(ls ${outfile_dir}/zen.${jd}.*.flag_waterfall_round_5.h5 2>/dev/null | wc -l)
if [ "${n_outfiles}" -gt 0 ]; then
    echo Found ${n_outfiles} round 5 flag waterfall file\(s\) in ${outfile_dir}.
else
    echo No round 5 flag waterfall files found in ${outfile_dir}.
    exit 1
fi
