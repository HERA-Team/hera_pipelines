...
# This function uploads all calibration notebooks and associated csv file to the Librarian
...
# 3 - librarian_calibration_notebooks: boolean trigger for this step
fn="${1}"
upload_to_librarian="${2}"
librarian_calibration_notebooks="${3}"
...
if [ "${upload_to_librarian}" == "True" ]; then
    if [ "${librarian_calibration_notebooks}" == "True" ]; then

        # Compress all calibration file per output type into one with a JD corresponding to $fn
        declare -a exts=(
            ".ant_class.csv"
            ".calibration_notebook.html"
        )
        for ext in ${exts[@]}; do
...
echo Finished running librarian calibration notebooks at $(date)
