...
# map assigns to one raw sum file: pseudo-Stokes I with the polarized source models subtracted, delay- and fringe-rate-filtered (under the delay-filtered round's flags too),
# saved as an SNR for the matching full-day pI RFI notebook (H6C SINGLE_BASELINE_PI_SNR_NOTEBOOK_WITHFRF_FRF).

src_dir="$(dirname "$0")"
source ${src_dir}/_common.sh

# Positional args (must match phase_II_analysis.toml [SINGLE_BASELINE_PI_FRF_SNR_NOTEBOOK])
fn=${1}
toml_file=${2}
nb_template_dir=${3}
nb_output_repo=${4}
...
export ACTION=SINGLE_BASELINE_PI_FRF_SNR_NOTEBOOK
...
# Execute notebook
nb_outfile=${SUM_FILE%.uvh5}.pI_FRF_SNR_notebook.html
...
if [ "${assignment}" == "publish" ]; then
    jd=$(get_int_jd ${fn})
    nb_dest_dir=${nb_output_repo}/single_baseline_pI_FRF_SNR
    nb_dest_file=${nb_dest_dir}/single_baseline_pI_FRF_SNR_${jd}.html
    mkdir -p ${nb_dest_dir}
